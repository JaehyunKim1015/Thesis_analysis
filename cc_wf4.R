...
## ==========================================
model_wf <- clogit(
  case ~ wildfire_flag + strata(stratum),
  weights = weight,
  data    = cc_all
  # method 인자 생략 → 기본값 "exact"
)

print(summary(model_wf))

beta <- coef(model_wf)["wildfire_flag"]
ci   <- confint(model_wf)["wildfire_flag", ]

OR  <- exp(beta)
LCL <- exp(ci[1])
UCL <- exp(ci[2])

cat("\n=== Wildfire_flag 효과 (lag 0, 단변량) ===\n")
cat("OR :", OR, "\n")
cat("95% CI:", LCL, "~", UCL, "\n")

## ---------- PAN 계산 (lag 0) ----------
total_cases   <- sum(cc_all$case == 1, na.rm = TRUE)
exposed_cases <- sum(cc_all$case == 1 & cc_all$wildfire_flag == 1, na.rm = TRUE)
...
fwrite(res_main,
       file.path(out_dir, "cc_wildfire_flag_lag0_result_with_PAN.csv"),
       bom = TRUE)
...
for (k in 0:max_lag) {
  ...
  if (k == 0) {
    cc_all[, (vname) := wildfire_flag]
  } else {
    cc_all[, (vname) := shift(wildfire_flag, n = k, type = "lag"), by = SGG_CD]
  }
}
...
for (k in 0:max_lag) {
  ...
  fit <- try(
    clogit(
      ...
      weights = weight,
      data    = dt
      # 여기서도 exact 쓰고 싶으면 method 생략 유지,
      # 예전 approximate 쓰려면 method = "approximate" 추가
    ),
    silent = TRUE
  )
...
fwrite(lag_results_dt,
       file.path(out_dir, "cc_wildfire_flag_lag0_12_results_with_PAN.csv"),
       bom = TRUE)

cat("\n✅ lag 0~12시간 wildfire_flag OR + PAN 결과 저장 완료\n")
