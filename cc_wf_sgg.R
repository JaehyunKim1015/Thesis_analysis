...
## ==========================================
## 1. 한 연도씩 case-crossover용 데이터 생성
## ==========================================
make_cc_data_one_year <- function(YR) {
  ...
  # wildfire_flag 0/1 보장
  if (!("wildfire_flag" %in% names(exp_dt))) {
    stop("Exposure에 wildfire_flag 컬럼이 없습니다. (year=", YR, ")")
  }
  exp_dt[, wildfire_flag := as.integer(wildfire_flag > 0)]
  ...
  ## ---------- 3) 노출 + NEDIS 병합 ----------
  merged <- merge(
    exp_dt[, .(SGG_CD, datetime, wildfire_flag, year, month, dow, hour)],
    ed_dt,
  ...
  cc_dt <- merged[!is.na(wildfire_flag)]
...
cc_all[, wildfire_flag := as.integer(wildfire_flag > 0)]
...
for (lg in lags) {
  ...
  # 3-1. Lag 변수 생성
  if (lg == 0) {
    cc_ordered[, wf_lag := wildfire_flag]
  } else {
    # SGG_CD별로 정렬된 순서에 따라 lag 계산
    cc_ordered[, wf_lag := shift(wildfire_flag, n = lg, type = "lag"), by = SGG_CD]
  }
  
  # Lag 변수 결측치 (각 SGG의 초기 lg시간) 제거
  ...
  for (i in seq_along(all_sgg_codes)) {
    sgg <- all_sgg_codes[i]
    foo <- dt_lag[SGG_CD == sgg & case == 1] # 케이스 발생 시점만 필터링
    
    # Case 발생 시점의 stratum 추출 (Control Period를 찾아 병합하기 위해)
    unique_strata <- unique(foo$stratum)
    
    # 해당 strata에 속하는 모든 시점(케이스 시점 + 대조 시점)을 원본에서 가져옴
    dt_sgg_cc <- dt_lag[stratum %in% unique_strata]
    
    n_case_val <- sum(dt_sgg_cc$case) # 실제 strata 내 case 수 (가중치 합이 아닌 count > 0인 행 수)
    
    # clogit 모델 실행 조건
    # 1. 해당 SGG에 케이스가 존재해야 함
    # 2. 노출 변수(wf_lag)가 최소 2개 이상의 값(0과 1)을 가져야 함
    if (n_case_val > 0 &&
      ...
      fit <- try(
        clogit(
          ...
          data = dt_sgg_cc,
          weights = weight, # 환자 수로 가중
          method = "exact" # exact 방법론 적용 (계산량 증가하나 안정적)
        ),
        silent = TRUE
      )
...
## 4. SGG 이름 붙여서 저장
## =========================================
# target_list에 sido, sigungu 이름이 없으므로, map_df_2024를 다시 로드하여 사용
map_file_2024 <- file.path(map_dir, "sgg_cd_2024.csv")
...
target_sgg_names <- unique(map_df_2024[, .(SGG_CD = code, SIDO_NM = sido, SGG_NM = sigungu)])
...
cat("성공적으로 모델링된 SGG 개수:", sum(final_res$note == "Success"), "\n")
print(final_res[note == "Success" & lag_hour == 0][1:5]) # 성공 결과 5개 예시 출력

out_file <- file.path(out_dir, "SGG_level_RR_2015_2024_wildfire_cc_hourly.csv")
