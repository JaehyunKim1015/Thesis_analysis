...
# 1. 데이터 불러오기 및 전처리
# -----------------------------------------------------------------------------
path_or <- "C:/Users/USER/Desktop/thesis/output1/SGG_level_OR_2015_2024_FILLED.csv"
...
# 결과 저장
output_path <- "C:/Users/USER/Desktop/thesis/output1/meta_regression_results_filtered.csv"
write.csv(final_results, output_path, row.names = FALSE)
print(final_results)

# 버블 플롯 (가장 유의한 변수)
best_var_row <- final_results %>% arrange(P_value) %>% slice(1)
best_var <- best_var_row %>% pull(Variable)

m_gen_best <- metagen(TE = logRR, seTE = seTE, data = filtered_data, sm="OR", method.tau="REML")
m_reg_best <- metareg(m_gen_best, as.formula(paste("~", best_var)))

# bubble() 함수는 meta 라이브러리의 플롯 함수입니다.
bubble(m_reg_best, xlab = paste(best_var, "(%)"), ylab = "Log Odds Ratio", main = paste("Meta-Regression Bubble Plot (Lag 1 day):", best_var))
